...
changes %>% filter(!is.na(Discipline), MAJORNUM == 1) %>% group_by(MAJORNUM) %>% mutate(y1t = sum(y1), y2t = sum(y2)) %>% mutate(cutoff = ifelse(MAJORNUM == 1, .05, .1)/100) %>% ungroup %>% arrange(MAJORNUM) %>% filter(y1/y1t > cutoff, y2/y2t > cutoff)  %>% 
  mutate(change = `y2` / `y1`) %>% 
...
ys = c(2003, 2007, 2013, 2017)

groups = data_frame(year = c(ys[1]:ys[2], ys[3]:ys[4]), period = c(rep("early", ys[2] - ys[1] + 1), rep("late", ys[4] - ys[3] + 1)))
...
changefunc = . %>% inner_join(groups) %>%
  group_by(UNITID) %>%
  filter(n() == 10) %>%
  group_by(UNITID, period) %>% summarize(degrees = sum(degrees))  %>% 
...
tuition = read_csv("ic2017_ay.csv") %>% mutate(costs = as.numeric(TUITION1) + as.numeric(FEE1)) %>% mutate(expensivo = costs > 20000) %>% select(UNITID, expensivo, costs)
...
#     log(other_hum_change)
   , data = modelable, weights = early + late) %>% summary
z
...
minimodel = lm(log(change) ~ label + control + HBCU + log(overall_change), 
               data = modelable, weights = early)
...
modelable$minipred = exp(predict(minimodel, newdata = modelable))
d = modelable %>% mutate(performance = change/pred) %>% arrange(-performance) %>% filter(late > 64)

d %>% ggplot() + 
  geom_point() + scale_x_log10() + scale_y_log10() + geom_text_repel(data = d %>% filter(tuning) %>% head(15), color='black', size=3) + aes(label = INSTNM, x=pred, y = change, color=tuning, size = early)
