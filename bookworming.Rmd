...
Some bad news: there's something weird going on with the relative numbers of public and private in 1998. I would guess some major public systems are split up or unified--eg, Wisconsin stops reporting all campuses and starts reporting each individual school, and now it shows up in my numbers. Conclusion: don't use pre-1997 data at institution level.
...
charelite = top30LA %>% rbind(top30) %>% right_join(characteristics)
...
collegecounts = school_counts %>% acadjoin %>% group_by(UNITID, Bachelors, `year < 2010`) %>% summarize(degrees = sum(degrees)) %>% group_by(UNITID, `year < 2010`) %>% mutate(school_count = sum(degrees), percent = degrees/school_count)
...
instidata = shares %>% mutate(change = `FALSE`/`TRUE`) %>% inner_join(counts) %>% inner_join(characteristics) %>% 
  mutate(
  college_class = 
    case_when(
      CARNEGIE %in% c(15, 16) ~ "Research Universities",
...
lm(log(change) ~ college_class_f + locale_f + control + hbcu, data = instidata) %>% summary
