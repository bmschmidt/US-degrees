...
Women's colleges are colleges that graduate women.

```{r}

ratio = data %>% filter(AWLEVEL==5, MAJORNUM == 1) %>% group_by(UNITID, gender) %>%
  summarize(count=sum(degrees, na.rm=T)) %>%
  summarize(female = sum(count[gender=="women"], na.rm=T)/sum(count, na.rm=T), tot = sum(count))

womens_colleges = ratio %>% ungroup %>% filter(female > .95) %>% arrange(-tot) %>% inner_join(characteristics) %>% transmute(female = TRUE, UNITID)
```
...
charelite = top30LA %>% rbind(top30) %>% right_join(characteristics) %>% left_join(womens_colleges)

charelite = charelite %>% mutate(
  label = 
    case_when(
      female ~ "Women's Colleges",
      elite ~ case_when(
...
```{r}
BYU = characteristics %>% filter(grepl("Air Force Academy|Naval Academy|Military Academy|Marine Academy|Coast Guard", INSTNM)) %>% inner_join(data) %>% filter(AWLEVEL==5, MAJORNUM==1) %>% acadjoin %>% group_by(INSTNM,  Discipline, Bachelors, year) %>% summarize(degrees=sum(degrees)) %>% filter(year >= 2003 || INSTNM != "United States Military Academy")
...
amadjusted %>% filter(Bachelors %>% grepl("Other",.))
```
...
elite_humanities = d4 %>% 
  ...
  group_by(label) %>%
  mutate(percent = (degrees/year_total)/(degrees/year_total)[year==2007])
...
collegecounts = school_counts %>%
  acadjoin %>% 
  group_by(UNITID, Bachelors, `year < 2010`) %>% 
  summarize(degrees = sum(degrees)) %>%
...
instidata %>% ungroup %>% mutate(p = predictions) %>% select(INSTNM, CARNEGIE, CONTROL, STABBR, old_count, new_count, old_share, new_share, change, p) %>% 
  arrange(-change)  %>% filter(new_share > .02) %>% filter(grepl("Pomona College|Ozarks|Princeton|Bob Jones|(Brigham Young.*Provo)|Liberty University|Wheaton College", INSTNM)) %>% arrange(-change) %>% mutate(change = change - 1)
...
characteristics %>% filter(INSTNM=="Harvard University") %>% select(UNITID) %>% inner_join(data) %>% filter(AWLEVEL==5, MAJORNUM==1) %>% cipsjoin() %>% acadjoin%>% group_by(Bachelors, Discipline, CIPTitle, year) %>% summarize(count=sum(degrees))  %>% ggplot() + geom_point(aes(x=year, y = count, color=Bachelors)) + facet_wrap(~CIPTitle)
```


```{r}
instidata %>% filter(old_share > .05) %>% arrange(-change)
```

```{r}

characteristics %>% filter(INSTNM  %>% grepl("Pomona College|Princeton University|Rutgers University.*Brunswick|Bob Jones University|Brigham Young University",.)) %>% inner_join(data) %>% filter(AWLEVEL==5, MAJORNUM==1, year > 2003) %>% acadjoin %>% group_by(INSTNM,year) %>% mutate(school_count = sum(degrees)) %>% group_by(Bachelors, school_count, add=T) %>% summarize(degrees = sum(degrees)) %>% mutate(percent = degrees/school_count) %>% filter(Bachelors=="Humanities") %>% ggplot() + geom_line(aes(x = year, y = percent)) + facet_wrap(~INSTNM, scales="free_y") + scale_y_continuous(labels=scales::percent) + expand_limits(y=0)
```
